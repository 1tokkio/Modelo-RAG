...
def verificar_rate_limit(ip: str) -> None:
    ahora = time.time()
    _rate_limit[ip] = [t for t in _rate_limit[ip] if ahora - t < RATE_LIMIT_VENTANA_SEG]
    if len(_rate_limit[ip]) >= RATE_LIMIT_MAX_SOLICITUDES:
        registrar_audit(ip, "RATE_LIMIT_BLOQUEADO",
                        f"solicitudes={len(_rate_limit[ip])} ventana={RATE_LIMIT_VENTANA_SEG}s")
        raise HTTPException(
            status_code=429,
