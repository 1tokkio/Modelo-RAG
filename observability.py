...
_proceso = psutil.Process() if _PSUTIL else None
...
def registrar_llamada(endpoint: str, pregunta: str, latencia_total_ms: float,
                      exito: bool, error: str = None, respuesta: str = "",
                      latencia_rag_ms: float = 0, latencia_llm_ms: float = 0) -> None:
    ...
    lat_media = sum(datos["latencias_ms"]) / len(datos["latencias_ms"]) if datos["latencias_ms"] else 0
...
def obtener_analisis() -> dict:
    ...
    errores_eventos = [e for e in eventos if not e.get("exito")]

    endpoint_mas_lento = ""
    lat_max_ep = 0
    for ep, count in datos["consultas_por_endpoint"].items():
        lats_ep = [e["latencia_ms"] for e in eventos if e["endpoint"] == ep]
        if lats_ep:
            avg = sum(lats_ep) / len(lats_ep)
            if avg > lat_max_ep:
                lat_max_ep = avg
    ...
    hora_pico = max(horas, key=horas.get) if horas else "N/A"

    umbral_anomalia = (_percentil(latencias, 95) if latencias else 0)

    patrones = []
    if anomalias:
        patrones.append(f"{len(anomalias)} consultas con latencia > 2× el promedio detectadas.")
    if errores_eventos:
        tipo_frecuente = max(datos["errores_por_tipo"], key=datos["errores_por_tipo"].get) if datos["errores_por_tipo"] else "N/A"
        patrones.append(f"Error más frecuente: {tipo_frecuente} ({datos['errores_por_tipo'].get(tipo_frecuente, 0)} ocurrencias).")
    if endpoint_mas_lento:
        patrones.append(f"Endpoint más lento: /{endpoint_mas_lento} ({round(lat_max_ep, 0)} ms promedio).")
    if hora_pico != "N/A":
        patrones.append(f"Hora pico de uso: {hora_pico} ({horas.get(hora_pico, 0)} consultas).")

    recomendaciones = []
    if datos.get("latencias_llm_ms") and datos.get("latencias_rag_ms"):
        avg_llm = statistics.mean(datos["latencias_llm_ms"]) if datos["latencias_llm_ms"] else 0
        avg_rag = statistics.mean(datos["latencias_rag_ms"]) if datos["latencias_rag_ms"] else 0
        if avg_llm > avg_rag * 3:
            recomendaciones.append("El LLM representa >75% de la latencia. Evaluar caché de respuestas frecuentes.")
        if avg_rag > 500:
            recomendaciones.append("Latencia RAG elevada (>500ms). Considerar reducir k=4 a k=2 o usar índice HNSW.")
    if datos["total_errores"] / max(datos["total_consultas"], 1) > 0.1:
        recomendaciones.append("Tasa de error >10%. Revisar validación de SKUs y conectividad con GitHub Models.")
    if len(anomalias) > 3:
        recomendaciones.append("Múltiples anomalías de latencia. Implementar circuit breaker y timeout explícito.")

    return {
        "total_anomalias": len(anomalias),
        "umbral_anomalia_ms": umbral_anomalia,
        "endpoint_mas_lento": endpoint_mas_lento,
        "latencia_promedio_endpoint_ms": round(lat_max_ep, 2),
        "hora_pico": hora_pico,
        "patrones_detectados": patrones,
        "recomendaciones": recomendaciones,
        "errores_recientes": [
            {"timestamp": e["timestamp"], "endpoint": e["endpoint"], "error": e["error"]}
            for e in errores_eventos[-5:]
        ],
    }
